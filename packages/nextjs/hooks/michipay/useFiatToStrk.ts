...
export const useFiatToStrk = () => {
  ...
  const convertFiatToStrkU256 = (amount: number, currency: 'USD' | 'MXN') => {
    ...
    const scaledAmountStr = (strkAmount * 1e18).toLocaleString('fullwide', { useGrouping: false });
   ...
   */
  const convertFiatToStrkDisplay = (amount: number, currency: 'USD' | 'MXN') => {
      const rate = currency === 'USD' ? exchangeRateUsd : exchangeRateMxn;
      if (!rate || amount <= 0) return null;
      return amount / rate;
  }

  return { exchangeRateUsd, exchangeRateMxn, isLoading, error, convertFiatToStrkU256, convertFiatToStrkDisplay };
};
