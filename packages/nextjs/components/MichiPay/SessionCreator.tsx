...
export const SessionCreator = () => {
  ...
  const { convertFiatToStrkU256, convertFiatToStrkDisplay, isLoading: ratesLoading } = useFiatToStrk();
  const { contacts, getSuggestions } = useAddressBook();
  
  const { data: deployedContractData } = useDeployedContractInfo("MichiPayContract");
  ...
  const getValidParticipants = () => participants.filter(p => p.startsWith("0x") && p.length > 20);
  const validParticipants = getValidParticipants();

  // Validation based on Cairo asserts (2 to 5 participants)
  const isValid = validParticipants.length >= 2 && validParticipants.length <= 5 && Number(amount) > 0;
    ...
    <div className="card bg-base-100 shadow-xl border border-primary/20">
      <div className="card-body">
        ...
        <div className="form-control w-full my-2">
          <label className="label">
            ...
            <span className="label-text-alt">
              {ratesLoading ? "Loading rates..." : `Est: ${convertFiatToStrkDisplay(Number(amount || 0), currency)?.toFixed(4) || "0.0000"} STRK`}
            </span>
          </label>
        ...
        <div className="flex justify-between items-center mb-2">
          ...
          <span className="text-xs opacity-70">{validParticipants.length}/5 Valid</span>
        </div>
        ...
        <div className="card-actions justify-end mt-6">
          ...
          >
            {isTxPending ? <span className="loading loading-spinner"></span> : "Create Session"}
          </button>
        </div>
