...
const SessionRow = ({
  ...
  return (
    <tr
      className="hover:bg-base-200 transition-colors cursor-pointer group"
      onClick={onClick}
    >
      <td className="font-semibold text-base-content group-hover:text-secondary transition-colors">
        {sessionId.toString()}
      </td>
      <td className="font-medium text-base-content/90">
        {formattedAmount} STRK
      </td>
      <td>
        <span
...
export const Dashboard = ({ onGoToCreate }: { onGoToCreate?: () => void }) => {
  ...
  // Load User Debt
  const { data: userDebtData, isLoading: isDebtLoading } =
  ...
  const { data: sessionCounterData, isLoading: isCounterLoading } =
    useScaffoldReadContract({
      contractName: "MichiPayContract",
  ...
  if (userDebtData !== undefined && userDebtData !== null) {
    const rawDebt = (
      Array.isArray(userDebtData) ? userDebtData[0] : userDebtData
    ) as any;
    const debtValue =
    ...
    return (
      <div className="card bg-base-100/70 shadow-xl mt-4 border border-base-200 backdrop-blur-md transition-all">
        <div className="card-body items-center justify-center text-center py-20">
          ...
          <h2 className="card-title text-4xl font-extrabold text-base-content tracking-tight mb-2">
            Bienvenido a MichiPay
          </h2>
          <p className="text-base-content/70 text-lg max-w-md">
            Conecta tu wallet en la esquina superior derecha para visualizar tu
            saldo y participar en tus sesiones pendientes. 💸
          </p>
        </div>
      </div>
    );
  ...
  if (sessionCounter > 0n) {
    const maxEntries = 5n;
    const start =
      sessionCounter > maxEntries ? sessionCounter - maxEntries + 1n : 1n;
    ...
    <>
      <div className="card bg-base-100/80 shadow-xl mt-4 border border-base-200 backdrop-blur-md">
        <div className="card-body p-6 md:p-8">
          <h2 className="card-title text-2xl mb-5 text-base-content font-bold">
            Tu Panel
          </h2>
          ...
          <div className="stats shadow-lg bg-secondary text-white border border-secondary/20 relative overflow-hidden">
            ...
            <div className="stat relative z-10 p-6">
              <div className="stat-title text-white/80 font-medium tracking-wide">
              ...
              <div className="stat-desc text-white/80 font-medium">
          ...
          <div className="mt-10">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-bold text-base-content mb-0">
                Sesiones Recientes
              </h3>
              {onGoToCreate && (
                <button
